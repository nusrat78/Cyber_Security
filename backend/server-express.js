  ...
  try {
    ...
    if (deviceExists) {
      ...
      db.run(
        ...
        [req.ip || 'unknown', req.headers['user-agent'] || '', deviceRow[0]]
      );
    } else {
      try {
        db.run(
          `INSERT INTO login_devices (user_id, device_id, device_name, device_type, ip_address, user_agent)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [user.id, deviceId, 'Unknown Device', 'Web', req.ip || 'unknown', req.headers['user-agent'] || '']
        );
      } catch (err) {
        db.run(
          `UPDATE login_devices SET last_seen = CURRENT_TIMESTAMP, ip_address = ?, user_agent = ? WHERE device_id = ? AND user_id = ?`,
          [req.ip || 'unknown', req.headers['user-agent'] || '', deviceId, user.id]
        );
      }
    }
...
app.post('/api/verify-2fa', async (req, res) => {
  const result = await handleVerify2FA(req.body);
  res.status(result.status).json(result.data);
});
